    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        # find the argmax of each distribution (the most likely position of each ghost)
        most_likely_positions = []
        for distribution in livingGhostPositionDistributions:
            most_likely_positions.append(distribution.argMax())
        # find the closest ghost to pacman
        closest_ghost = min(most_likely_positions, key=lambda ghost: self.distancer.getDistance(pacmanPosition, ghost))
        # closest_ghost = None
        ...
        return_action = min(legal, key=lambda legal_action: self.distancer.getDistance(closest_ghost, Actions.getSuccessor(pacmanPosition, legal_action)))
