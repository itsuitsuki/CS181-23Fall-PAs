...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: BayesNet, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for elim in eliminationOrder:
            # join variable: elim
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, elim)
            if len(joinedFactor.unconditionedVariables()) != 1:  # P(+a|B,C,D) can also happen
                currentFactorsList.append(eliminate(joinedFactor, elim))
