...
from hunters import GHOST_COLLISION_REWARD, WON_GAME_REWARD, GameState
...
def constructBayesNet(gameState: GameState):
    """
    ...
    "*** YOUR CODE HERE ***"
    obsVars.extend([OBS_VAR_TEMPLATE % obsPos 
                    for housePos in gameState.getPossibleHouses() 
                    for obsPos in gameState.getHouseWalls(housePos)])
    # obsVars.append(X_POS_VAR)
    # obsVars.append(Y_POS_VAR)
    ...
    variableDomainsDict.update({obsVar: OBS_VALS for obsVar in obsVars})
    
    edges.extend([(X_POS_VAR, FOOD_HOUSE_VAR), (X_POS_VAR, GHOST_HOUSE_VAR)])
    edges.extend([(Y_POS_VAR, FOOD_HOUSE_VAR), (Y_POS_VAR, GHOST_HOUSE_VAR)])
    edges.extend([(FOOD_HOUSE_VAR, obsVar) for obsVar in obsVars])
    edges.extend([(GHOST_HOUSE_VAR, obsVar) for obsVar in obsVars])
...
def fillYCPT(bayesNet: BayesNet, gameState: GameState):
    """
...
def fillHouseCPT(bayesNet: BayesNet, gameState: GameState):
...
def fillObsCPT(bayesNet: BayesNet, gameState: GameState):
    """
...
def getMostLikelyFoodHousePosition(evidence: Dict, bayesNet: BayesNet, eliminationOrder: List[str]):
    """
    ...
    """
    "*** YOUR CODE HERE ***"
    queries = [FOOD_HOUSE_VAR]
    food_house_factor = inference.inferenceByVariableElimination(bayesNet, queries, evidence, eliminationOrder)
    all_assignments = food_house_factor.getAllPossibleAssignmentDicts()
    max_prob_assignment = max(all_assignments, key=food_house_factor.getProbability)
    # print (max_prob_assignment)
    return max_prob_assignment
    "*** END YOUR CODE HERE ***"
